...
export class MockSystemAdapter implements SystemAdapter {
  ...
  // Settings
  setDND(enabled: boolean): void { this.dnd = enabled; }
  getDND(): boolean { return this.dnd; }
  setBrightness(percentage: number): void { this.brightness = percentage; }
  getBrightness(): number { return this.brightness; }
  async setVolume(percentage: number): Promise<void> { this.volume = percentage; }
  async getVolume(): Promise<number> { return this.volume; }
  async setBluetoothVolume(percentage: number): Promise<boolean> { return true; }
  setSinkVolume(sinkName: string, percentage: number): void {}
  getBluetoothAudioSinkName(): string | null { return null; }
  setWallpaper(uri: string): void {}
  getWallpaper(): string { return ''; }
  ...
  async getBluetooth(): Promise<boolean> { return this.bluetoothState; }

  // Connectivity
  setWifi(enabled: boolean): void { this.wifiState = enabled; }
  async connectToWifi(ssid: string): Promise<boolean> { this.connectedWifi = ssid; return true; }

  // State
  getWifiState(): boolean { return this.wifiState; }
  getBatteryLevel(): number { return 100; }
  isCharging(): boolean { return false; }

  // App Tracking
  getActiveApp(): string | null { return null; }
  ...
  getCurrentWifiSSID(): string | null { return this.connectedWifi; }
  getSavedWifiNetworks(): string[] { return ['Home WiFi', 'Office WiFi']; }

  // Wifi Power
  getWifiPowerState(): boolean { return this.wifiState; }
  ...
  setAirplaneMode(enabled: boolean): void { this.airplaneMode = enabled; }

  // New Actions - Display
  setDarkMode(enabled: boolean): void { this.darkMode = enabled; }
  getDarkMode(): boolean { return this.darkMode; }
  setNightLight(enabled: boolean): void { this.nightLight = enabled; }
  getNightLight(): boolean { return this.nightLight; }
  setScreenTimeout(seconds: number): void {}
  getScreenTimeout(): number { return 60; }
  setScreenOrientation(orientation: 'portrait' | 'landscape'): void {}
  setRefreshRate(rate: number): void {}
  getRefreshRate(): number { return 60; }
  getAvailableRefreshRates(): number[] { return [60, 144]; }

  // New Actions - Power
  setPowerSaver(enabled: boolean): void { this.powerSaver = enabled; }
  getPowerSaver(): boolean { return this.powerSaver; }
  ...
  async getKeyboardBrightness(): Promise<number> { return 50; }

  // Clipboard
  async getClipboardContent(): Promise<{ type: 'text' | 'image' | 'other'; content?: string; }> {
