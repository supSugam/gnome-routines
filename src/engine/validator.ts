    ...
    if (routine.matchType === RoutineMatchType.ALL) {
      ...
      if (wifiTriggers.length > 1) {
        const hasConnect = wifiTriggers.some(
          (t) => (t.config as WifiTriggerConfig).state === ConnectionState.CONNECTED
        );
        const hasDisconnect = wifiTriggers.some(
          (t) =>
            (t.config as WifiTriggerConfig).state === ConnectionState.DISCONNECTED
        );
        const hasEnabled = wifiTriggers.some(
          (t) => (t.config as WifiTriggerConfig).state === ConnectionState.ENABLED
        );
        const hasDisabled = wifiTriggers.some(
          (t) => (t.config as WifiTriggerConfig).state === ConnectionState.DISABLED
        );
      ...
      if (btTriggers.length > 1) {
          ...
          const hasEnabled = btTriggers.some(
            (t) => (t.config as BluetoothTriggerConfig).state === ConnectionState.ENABLED
          );
          const hasDisabled = btTriggers.some(
            (t) => (t.config as BluetoothTriggerConfig).state === ConnectionState.DISABLED
          );
          ...
          // Check Conflicts per Device
           ...
           const connectConfigs = btTriggers.filter(t => (t.config as BluetoothTriggerConfig).state === ConnectionState.CONNECTED);
           const disconnectConfigs = btTriggers.filter(t => (t.config as BluetoothTriggerConfig).state === ConnectionState.DISCONNECTED);
           ...
           for (const conn of connectConfigs) {
               ...
               for (const disc of disconnectConfigs) {
                   ...
                   // Check intersection
                   const conflict = cConf.deviceIds.some(id => dConf.deviceIds?.includes(id));
                   if (conflict) {
                        return {
                            valid: false,
                            error: "Conflict: Cannot require both Connected AND Disconnected state for the same Bluetooth device in ALL match."
                        };
                   }
               }
           }
