...
export class RefreshRateAction extends BaseAction {
    ...
    constructor(id: string, config: { rate: number }, adapter: SystemAdapter, stateManager: StateManager, routineId: string) {
    ...
    async revert(): Promise<void> {
        const savedRate = this.stateManager.restoreState(this.routineId, 'refresh_rate');
        if (savedRate !== null) {
