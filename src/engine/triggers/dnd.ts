...
export class DndTrigger extends BaseTrigger {
  ...
  activate(): void {
    debugLog(`[DndTrigger] Activating listener. State: ${this.config.state}, Strategy: ${this.strategy}`);
    ...
    this.check().then((initialState) => {
      if (!this._initialized) {
        if (shouldIgnoreInitial) {
           debugLog(`[DndTrigger] Initial State: ${initialState} (Baselined - Ignored by Strategy)`);
           this._lastMatchState = initialState;
           this._initialized = true; 
        } else {
           debugLog(`[DndTrigger] Initial State: ${initialState} (Checking for immediate trigger)`);
           this._lastMatchState = initialState;
           this._initialized = true;
    ...
    if (this.adapter.onDndStateChanged) {
        this.cleanup = this.adapter.onDndStateChanged(async (isDnd) => {
            ...
            if (!this._initialized) {
                 ...
                 if (shouldIgnoreInitial) {
                    debugLog(`[DndTrigger] Initial Event: ${isMatch} (Baselined - Ignored)`);
                    this._lastMatchState = isMatch;
                    this._initialized = true;
                    return;
                 } else {
                    debugLog(`[DndTrigger] Initial Event: ${isMatch} (Checking immediate)`);
                    this._lastMatchState = isMatch;
                    this._initialized = true;
                    if (isMatch) {
                         this.emit('triggered');
            ...
            if (isMatch !== this._lastMatchState) {
                debugLog(`[DndTrigger] State transition: ${this._lastMatchState} -> ${isMatch}`);
                this._lastMatchState = isMatch;

                if (isMatch) {
                    debugLog(`[DndTrigger] Condition met (TRUE). Emitting 'triggered'.`);
                } else {
                    debugLog(`[DndTrigger] Condition lost (FALSE). Emitting 'triggered' to signal update.`);
                }
                this.emit('triggered');
            }
        });
