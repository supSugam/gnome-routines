...
export class RoutineManager implements RoutineManagerInterface {
    ...
    async load() {
        ...
        rawRoutines.forEach(r => {
            const triggers = r.triggers.map(t => TriggerFactory.create(t)).filter(t => t !== null) as Trigger[];
            const actions = r.actions.map(a => ActionFactory.create(a, this.adapter)).filter(a => a !== null) as Action[];
    ...
    addRoutine(rawRoutine: Routine): void {
        // Hydrate new routine
        const triggers = rawRoutine.triggers.map(t => TriggerFactory.create(t)).filter(t => t !== null) as Trigger[];
        const actions = rawRoutine.actions.map(a => ActionFactory.create(a, this.adapter)).filter(a => a !== null) as Action[];
    ...
    async evaluate(): Promise<void> {
        for (const routine of this.routines.values()) {
            ...
            const shouldBeActive = await this.checkTriggers(routine.triggers, routine.matchType || 'all');
            ...
            if (trigger.activate && !trigger._isActivated) {
                ...
                if (trigger.on) {
                    trigger.on('triggered', () => {
                        console.log(`[RoutineManager] Trigger ${trigger.id} fired for routine ${routine.name}`);
                        this.evaluate();
                    });
    ...
    private async checkTriggers(triggers: Trigger[], matchType: 'any' | 'all'): Promise<boolean> {
    ...
    private async activateRoutine(routine: Routine) {
        ...
        for (const action of routine.actions) {
            try {
                ...
                console.error(`Failed to execute action ${action.id} in routine ${routine.name}:`, e);
            }
        }
    }
        ...
        for (let i = routine.actions.length - 1; i >= 0; i--) {
            ...
            if (action.revert) {
                try {
                    ...
                    console.error(`Failed to revert action ${action.id} in routine ${routine.name}:`, e);
                }
            }
        }
    }
