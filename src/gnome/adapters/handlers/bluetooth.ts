            ...
            try {
                ...
                proc.wait_check_async(null, (proc: any, res: any) => {
                    try {
                        ...
                        debugLog(
                            `[BluetoothAdapter] Bluetooth ${enabled ? 'enabled' : 'disabled'
                            } via bluetoothctl`
                        );
                    } catch (e) {
                        debugLog(
    ...
    onBluetoothPowerStateChanged(callback: (isEnabled: boolean) => void): () => void {
        try {
            ...
            const signalId = Gio.DBus.system.signal_subscribe(
                ...
                null,
                0,
                (connection: any, sender: any, path: any, iface: any, signal: any, params: any) => {
                    try {
                        ...
                        debugLog(
                            `[BluetoothAdapter] Error parsing DBus signal: ${err}`
                        );
                    }
                }
            );
            ...
            debugLog(
                '[BluetoothAdapter] Failed to subscribe to Bluetooth power:',
                e
            );
            return () => { };
        }
                        ...
                        try {
                            ...
                            for (const objectPath in objects) {
                                ...
                                if ('org.bluez.Device1' in interfaces) {
                                    ...
                                    if (connected) {
                                        const name = unpackVariant(deviceProps.Name) || 'Unknown Device';
